...
import { fetchEvaluationsByStudentIds, deleteEvaluation } from '../services/evaluationService';
      ...
      <div style={{ paddingTop: '80px', paddingBottom: '40px', flex: 1 }}>
        <Container style={{ maxWidth: '935px' }}>
          ...
          {selectedStudent && studentEvaluations.length > 0 && (
            <>
              <h5 className="mb-3">Relatorio de {getStudentName()}</h5>
              <Row className="g-3">
                <Col xs={12} md={6} lg={3}>
                  <Card className="border-0 shadow-sm h-100">
                    <Card.Body>
                      <h6 className="text-muted">Total de Avaliacoes</h6>
                ...
                <Col xs={12} md={6} lg={3}>
                  <Card className="border-0 shadow-sm h-100">
                    <Card.Body>
                      <h6 className="text-muted">Ultima Avaliacao</h6>
              ...
              <Card className="mt-4 border-0 shadow-sm">
                <Card.Header className="bg-white border-bottom py-3">
                  <h6 className="mb-0 fw-bold">Historico de Avaliacoes</h6>
                </Card.Header>
                <Card.Body>
                      ...
                      <Card.Body className="p-2">
                        <div className="d-flex justify-content-between align-items-start">
                          <div className="flex-grow-1">
                            ...
                            <div className="small text-muted">
                              {new Date(evaluation.date).toLocaleDateString('pt-BR')} - {evaluation.professional}
                            </div>
                          </div>
                          <div className="d-flex gap-1 align-items-start">
                            <Badge bg="light" text="dark" className="border">
                              {evaluation.complexityLevel}
                            </Badge>
                            <Button
                              size="sm"
                              variant="link"
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                if (confirm('Deseja excluir esta avaliacao?')) {
                                  ...
                                  if (success) {
                                    setEvaluations((prev) => prev.filter((entry) => entry.id !== evaluation.id));
                                  }
                                }
                              }}
                              className="text-decoration-none p-0 mx-1"
                        ...
                        {evaluation.attentionPoints && (
                          <div className="mt-1 small">
                            <strong className="text-warning">Atencao:</strong> {evaluation.attentionPoints}
                          </div>
                        )}
                      </Card.Body>
                    </Card>
          ...
          {selectedStudent && studentEvaluations.length === 0 && (
            <div className="alert alert-light border text-center mt-4">Nenhuma avaliacao encontrada para este aluno.</div>
          )}
        </Container>
      </div>
